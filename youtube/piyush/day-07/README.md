...
```bash
kubectl run nginx-1 --image=nginx:latest --dry-run=client
```

this will not create pod & output is given in YAML and creates pod.yaml
```bash
kubectl run nginx-1 --image=nginx:latest --dry-run=client -o yaml > pod.yaml
```

this will not create pod & output is given in json and creates pod.json
```bash
kubectl run nginx-1 --image=nginx:latest --dry-run=client -o json > pod.json
```
